...
const mockRides: Ride[] = mockRidesRaw.map(ride => {
  try {
    const pricePerPerson = calculatePricePerPerson(ride.vehicle, ride.distance, ride.totalSeats);
    return {
      ...ride,
      pricePerPerson
    };
  } catch (error) {
    console.error('Error calculating price for ride:', ride.id, error);
    return {
      ...ride,
      pricePerPerson: 5.00 // Fallback
    };
  }
});
